...
async function renderCharts() {
    ...
    for (const stock of tickers) {
        ...
        if (data) {
            if (labels.length === 0) labels = data.dates; // Используем даты с первого графика
            datasets.push({
...
// Генерация случайного цвета для линий графиков
...
async function updateStockList() {
    ...
    stockList.innerHTML = ''; // Очистить список перед обновлением
...
// Функция для получения текущей цены с MOEX API
...
// Функция для получения исторических данных о стоимости акций
async function fetchHistoricalStockData(ticker) {
    ...
    return data; // Возвращаем массив с историческими данными
}

// Функция для отображения текущей информации о цене акций
...
// Функция для построения графика стоимости акций
...
document.addEventListener('DOMContentLoaded', () => {
    displayCurrentStockData('SBER'); // Тикер акций
